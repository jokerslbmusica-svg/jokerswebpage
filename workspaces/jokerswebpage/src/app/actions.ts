...
let adminServices: FirebaseAdmin | null = null;

function getAdminServices(): FirebaseAdmin | null {
    if (adminServices) {
...
async function uploadFileToStorage(file: File, path: string) {
    ...
    const bucket = services.storage.bucket(bucketName);
...
export async function uploadBandMedia(formData: FormData): Promise<MediaItem> {
  const services = getAdminServices();
  if (!services) throw new Error("La base de datos no está disponible.");
  
  const imageUrl = formData.get("imageUrl") as string | null;
  const videoUrl = formData.get("videoUrl") as string | null;

  if (imageUrl) {
    const docRef = await services.db.collection(BAND_GALLERY_COLLECTION).add({
        name: 'Band Image',
        url: imageUrl,
        type: 'image',
        createdAt: new Date(),
...
export async function getBandMedia(): Promise<MediaItem[]> {
    ...
    try {
        const snapshot = await services.db.collection(BAND_GALLERY_COLLECTION).orderBy("createdAt", "desc").get();
        ...
        return []; // Return empty array on errors to prevent crash
    }
}
...
export async function deleteBandMedia(itemId: string): Promise<void> {
  ...
  await services.db.collection(BAND_GALLERY_COLLECTION).doc(itemId).delete();
  revalidatePath("/");
}
...
export async function saveBandBio(bioText: string): Promise<void> {
  ...
  const bioRef = services.db.collection(BAND_INFO_COLLECTION).doc(BIO_DOC_ID);
...
export async function getBandBio(): Promise<string | null> {
    ...
    try {
        const docSnap = await services.db.collection(BAND_INFO_COLLECTION).doc(BIO_DOC_ID).get();
        ...
        if (error.code === 5) { // NOT_FOUND error
            ...
            console.error("Error fetching band bio:", error);
        }
        return null; // Return null on errors to prevent crash
    }
}
...
export async function addFanComment(commentData: Omit<FanComment, 'id' | 'createdAt' | 'status'>): Promise<void> {
    ...
    await services.db.collection(FAN_COMMENTS_COLLECTION).add({
        ...commentData,
        status: "pending",
        createdAt: new Date(),
    });
...
export async function getFanComments(): Promise<FanComment[]> {
    ...
    try {
        const snapshot = await services.db.collection(FAN_COMMENTS_COLLECTION).orderBy("createdAt", "desc").get();
        if (snapshot.empty) return [];
        return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => {
            const data = doc.data();
            return {
        ...
        if (error.code === 5) { // NOT_FOUND error, collection likely doesn't exist
            ...
            console.error("Error fetching fan comments:", error);
        }
        return [];
    }
}
...
export async function deleteFanComment(commentId: string): Promise<void> {
    ...
    await services.db.collection(FAN_COMMENTS_COLLECTION).doc(commentId).delete();
    revalidatePath("/");
    revalidatePath("/admin");
}
...
export async function approveFanComment(commentId: string): Promise<void> {
    ...
    if (!commentId) throw new Error("Comment ID not provided.");
    await services.db.collection(FAN_COMMENTS_COLLECTION).doc(commentId).update({ status: "approved" });
    revalidatePath("/");
    revalidatePath("/admin");
}
...
export async function addFanMedia(formData: FormData): Promise<MediaItem> {
    ...
    const docRef = await services.db.collection(FAN_GALLERY_COLLECTION).add({
        name: "Fan Image",
        url: url,
...
export async function getFanMedia(): Promise<MediaItem[]> {
    ...
    try {
        const snapshot = await services.db.collection(FAN_GALLERY_COLLECTION).orderBy("createdAt", "desc").get();
        if (snapshot.empty) return [];
        return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({
            id: doc.id,
            ...doc.data()
...
export async function deleteFanMedia(itemId: string): Promise<void> {
    ...
    if (!itemId) throw new Error("Item ID not provided.");
    await services.db.collection(FAN_GALLERY_COLLECTION).doc(itemId).delete();
    revalidatePath("/");
    revalidatePath("/admin");
}
...
export async function addSong(formData: FormData): Promise<void> {
    ...
    await services.db.collection(MUSIC_COLLECTION).add({
        title,
        artist,
...
export async function getSongs(): Promise<Song[]> {
    ...
    try {
        const snapshot = await services.db.collection(MUSIC_COLLECTION).orderBy("createdAt", "desc").get();
        if (snapshot.empty) return [];
        return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({ id: doc.id, ...doc.data() })) as Song[];
    } catch (error: any) {
        if (error.code === 5) { // NOT_FOUND error, collection likely doesn't exist
...
export async function deleteSong(song: Song): Promise<void> {
    ...
    await Promise.all([
        bucket.file(song.audioPath).delete().catch((e: Error) => console.error(`Failed to delete audio file: ${e.message}`)),
        bucket.file(song.coverPath).delete().catch((e: Error) => console.error(`Failed to delete cover file: ${e.message}`)),
        services.db.collection(MUSIC_COLLECTION).doc(song.id).delete(),
    ]);
...
export async function addTourDate(formData: FormData): Promise<void> {
    ...
    await services.db.collection(TOUR_DATES_COLLECTION).add({ postUrl, createdAt: new Date() });
    revalidatePath("/");
    revalidatePath("/admin");
}
...
export async function getTourDates(): Promise<TourDate[]> {
    ...
    try {
        const snapshot = await services.db.collection(TOUR_DATES_COLLECTION).orderBy("createdAt", "desc").get();
        if (snapshot.empty) return [];
        return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({ id: doc.id, ...doc.data() })) as TourDate[];
    } catch (error: any) {
        if (error.code === 5) { // NOT_FOUND error, collection likely doesn't exist
...
export async function deleteTourDate(dateId: string): Promise<void> {
    ...
    await services.db.collection(TOUR_DATES_COLLECTION).doc(dateId).delete();
    revalidatePath("/");
    revalidatePath("/admin");
}

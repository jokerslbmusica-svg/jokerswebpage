...
import { Download, Trash2, Loader2, Camera, Youtube, ImageIcon, Link as LinkIcon } from "lucide-react";
...
export function BandGallery({ readOnly = false }: BandGalleryProps) {
  ...
  const handleUpload = async (formData: FormData) => {
    ...
    try {
      ...
      // No need to re-fetch if optimistic update is successful
    } catch (error: any) {
      console.error("Error uploading:", error);
      toast({ variant: "destructive", title: "Error de carga", description: error.message || "No se pudo subir el contenido." });
      // Re-fetch to get the true state from the server on error
      fetchMedia();
    } finally {

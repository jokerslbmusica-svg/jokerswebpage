    ...
    const handleDownloadZip = async () => {
        ...
        try {
            ...
            const imagePromises = imageItems.map(async (image) => {
                try {
                    // This uses a CORS proxy to fetch images from other origins.
                    // Replace with a more direct method if your images are hosted on the same origin or have CORS enabled.
                    const response = await fetch(`https://cors-anywhere.herokuapp.com/${image.url}`);
                    if (!response.ok) {
                        throw new Error(`Failed to fetch image: ${response.statusText}`);
                    }
                    const imageBlob = await response.blob();
                    const fileName = `JokersLiveBand_${image.id}.${imageBlob.type.split('/')[1] || 'jpg'}`;
                    zip.file(fileName, imageBlob);

                } catch (fetchError) {
                    console.error(`Failed to process image ${image.id}:`, fetchError);
                    // Optionally, skip failing images
                }
            });
